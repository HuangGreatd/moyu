...
import static com.juzipi.constant.UserConstant.SA_TOKEN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public String login(UserLoginRequest userLoginRequest, HttpServletRequest request) {
        ...
        stringRedisTemplate.opsForValue().set(USER_LOGIN_STATE + ":" + tokenValue, tokenInfoStr);
        StpUtil.login(userAccount);
//		request.getSession().setAttribute(USER_LOGIN_STATE, user);
    ...
    public UserVO getLoginUserVo(HttpServletRequest request) {
        String tokenValue ="11";
        System.out.println("tokenValue = " + tokenValue);
        Object o = StpUtil.getSession().get(SA_TOKEN_STATE);
        Object attribute = request.getSession().getAttribute(SA_TOKEN_STATE);
...
//        }
//        Gson gson = new Gson();
//        String tokenInfoStr = stringRedisTemplate.opsForValue().get(USER_LOGIN_STATE + ":" + tokenValue);
//        SaTokenInfo saTokenInfo = gson.fromJson(tokenInfoStr, SaTokenInfo.class);
//        String userAccount = (String) saTokenInfo.getLoginId();
//        User currentUser = userMapper.selectUserByUserAccount(userAccount);
//        if (currentUser == null) {
//            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
//        }
//        return convertToUserVO(currentUser);
    ...
    public boolean userLogout(HttpServletRequest request) {
        String satoken = (String) StpUtil.getSession().get("satoken");
        if (request.getSession().getAttribute("satoken") == null) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "未登录");
        }

        // 移除登录态
        stringRedisTemplate.delete(USER_LOGIN_STATE + ":" + satoken);
        StpUtil.logout();
        return true;
    }
